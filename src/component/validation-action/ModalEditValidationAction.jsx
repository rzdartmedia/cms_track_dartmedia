...
const ModalEditValidationAction = (props) => {
  ...
  useEffect(() => {
    if (props.validationId) {
      clearForm()
      getValidationById(props.validationId)
    }
  }, [props.modalEdit])

  async function getValidationById(id) {
    const result = await props.getValidationActionById(id)
    setIdValidation(result.idValidation)
    setNameAction(result.nameAction)
    setAction(result.action)
    setManyTimes(result.manyTimes)

    await result.channel?.map((data) => {
      switch (data.channel) {
        case "sms":
          setMessageChannelSms(data.message)
          setBoxChannelSms(true)
          break
        case "email":
          setTitleChannelEmail(data.title)
          setMessageChannelEmail(data.message)
          setBoxChannelEmail(true)
          break
        default:
          setTitleChannelApp(data.title)
          setMessageChannelApp(data.message)
          setBoxChannelApp(true)
      }
    })
  ...
  async function submitValidationAction(e) {
    ...
    const data = { idValidation, manyTimes, channel: dataChannel }
    const { errors: errorValidate, formIsValid } =
      ValidationActionValidator.validateUpdateValidationAction(data)
    ...
    const result = await props.updateValidationActionById(data)
    if (result) clearForm()
  }
            ...
            <div className='max-w-lg m-auto space-y-3 my-4 text-sm'>
              <div>
                ...
                <input
                  ...
                  } rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:text-gray-300 read-only:bg-gray-100 hover:cursor-default`}
                  id='nameAction'
                  type='text'
                  placeholder='Top Up'
                  value={nameAction}
...
export default ModalEditValidationAction
